...
RUN cd ${IROOT}/ffead-cpp-src/ && rm -rf CMakeFiles CMakeCache.txt build && mkdir build && cd build && emcmake cmake -DBUILD_SHARED_LIBS=off -DMOD_SDORM_SQL=off .. \
	&& emmake make -j4 install && rm -rf ${IROOT}/ffead-cpp-7.0 && mv ../ffead-cpp-7.0-bin ${IROOT}/ffead-cpp-7.0

WORKDIR ${IROOT}/lang-server-backends/javascript/emscripten

RUN emcc ffead-cpp-glue.cpp ${IROOT}/ffead-cpp-7.0/lib/libffead-framework.a ${IROOT}/ffead-cpp-7.0/lib/libffead-modules.a -s USE_PTHREADS=1 \
