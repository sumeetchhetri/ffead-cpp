...
Task::Task() :
	tunit(-1), type(-1), cleanUp(false), isFuture(false), console(false), priority(false) {
}
Task::Task(const int& priority) :
	tunit(-1), type(-1), cleanUp(false), isFuture(false), console(false) {
...
void Task::setCleanUp(const bool& cleanUp)
{
