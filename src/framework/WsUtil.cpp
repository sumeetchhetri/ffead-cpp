...
vector<WsDetails> WsUtil::getWsDetails(const vector<string>& apps, const string& serverRootDirectory)
{
	vector<WsDetails> wsdvec;

	for(unsigned int var = 0; var < apps.size(); ++var)
	{
		string webdir = serverRootDirectory+"/web/"+apps.at(var);
		string file = webdir+"/config/ws.xml";
...
void WsUtil::handleWebService(string& ws_funcs, const WsDetails& wsd, map<string, map<string, ClassStructure> >& clsstrucMaps, const string& resp, string &headers, Reflection& ref)
{
	typedef map<string,string> strMap;
	typedef map<string,strMap> meth_Map;
	typedef map<string,meth_Map> ws_inp_out_Map;
	string claz = wsd.claz;
	string location = wsd.location;
	string namespc = wsd.namespc;
	map<string, string> outnmmp = wsd.outnmmp;
	string appname = wsd.appname;
	map<string, ClassStructure>& allclsmap = clsstrucMaps[appname];
	...
	if(clstruct==NULL)
	{
		logger << ("Error generating web-service artifacts, as class "+claz + "not found...") << endl;
		return;
	}
				...
				if(iter2->first!="RETURN" && iter2->first!="RETURNTYP")
				{
					...
					else if(iter2->second.find("vector<")!=string::npos)
					{
						...
						else
						{
							...
							ws_funcs.append("ElementList list = _req->getElementsByName(\""+argname+"\");\n");
							ws_funcs.append("for(int i=0;i<list.size();i++)\n{\n");
							ws_funcs.append(argname+".push_back(XMLSerialize::unserialize<vector<"+vecn+" > >(list.at(i), \"vector<"+vecn+" >\", \""+appname+"\"));\n}\n");
						}
					}
					else
					{
			...
			else if(pars["RETURNTYP"]!="")
			{
				...
				if(pars["RETURNTYP"].find("vector<")!=string::npos)
				{
					...
					else
					{
						...
						//ws_funcs.append("\ncout << XMLSerialize::serialize<"+pars["RETURNTYP"]+" >(&(_retval.at(i)),\""+vecn+"\",\""+appname+"\") << endl;");
