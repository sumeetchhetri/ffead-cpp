...
TeBkUmLpqQwAsyncFortune::TeBkUmLpqQwAsyncFortune(int id, const uint8_t * buf, size_t len) {
	this->id = id;
...
TemplateJson TeBkUmLpqQwAsyncMessage::tJ = TemplateJson("{\"message\":\"\"}", {12});
...
void TeBkUmLpqQwAsyncRouter::fortunes(BaseSocket* sif) {
	...
	q->withSelectQuery(FORTUNE_ALL_QUERY).withContext(sif).withCb0([](void* ctx, PGresult* res) {
		...
		sif->queueWrite([](BaseSocket* sif, void* arg) {
			...
			flst->push_back(new TeBkUmLpqQwAsyncFortune(0, "Additional fortune added at request time."));
			std::sort(flst->begin(), flst->end(), [](const TeBkUmLpqQwAsyncFortune* a, const TeBkUmLpqQwAsyncFortune* b) {
