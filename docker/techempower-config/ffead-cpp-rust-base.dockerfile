...
RUN cd ${IROOT}/lang-server-backends/rust/actix-ffead-cpp && RUSTFLAGS="-C target-cpu=native" cargo build --release && cp target/release/actix-ffead-cpp $IROOT/ && rm -rf target && \
	...
	cd ${IROOT}/lang-server-backends/rust/rocket-ffead-cpp && rustup default nightly && cargo update && RUSTFLAGS="-C target-cpu=native" cargo build --release && cp target/release/rocket-ffead-cpp $IROOT/ && \
