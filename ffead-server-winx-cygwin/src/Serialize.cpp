...
void* Serialize::_hanldeAllUnSerialization(string objXml,string className)
{
	...
	else if(root->getNameStr().find("queue<")==0)
	{
...
void* Serialize::unserializeq(AMEFObject* root, string objXml)
{
	...
	if(t!=NULL)
	{
		...
		{
			if(className=="std::string")
				((queue<string>*)t)->push(root->getPackets().at(var)->getValueStr());
			else if(className=="int")
				((queue<int>*)t)->push(root->getPackets().at(var)->getIntValue());
			else if(className=="long")
				((queue<long>*)t)->push(root->getPackets().at(var)->getLongValue());
			else if(className=="float")
				((queue<float>*)t)->push(root->getPackets().at(var)->getFloatValue());
			else if(className=="double")
				((queue<double>*)t)->push(root->getPackets().at(var)->getDoubleValue());
			else if(className=="bool")
				((queue<bool>*)t)->push(root->getPackets().at(var)->getBoolValue());
		}
		return t;
	}
	return NULL;
