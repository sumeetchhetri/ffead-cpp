...
*/
/*
 * PooledDistoCacheConnectionFactory.h
 *
 *  Created on: 04-Apr-2013
 *      Author: sumeetc
 */

#ifndef POOLEDDISTOCACHECONNECTIONFACTORY_H_
#define POOLEDDISTOCACHECONNECTIONFACTORY_H_
...
class PooledDistoCacheConnectionFactory {
	...
	PooledDistoCacheConnectionFactory(string host, int port, int poolSize, bool isSSL= false);
	void createPool();
public:
	virtual ~PooledDistoCacheConnectionFactory();
	static void init(string host, int port, int poolSize, bool isSSL= false);
	static void destroy();
