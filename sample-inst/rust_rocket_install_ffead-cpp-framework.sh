...
cmake -DSRV_EMB=on -DDEBUG=on -DMOD_APACHE=off -DMOD_NGINX=off -DMOD_MEMCACHED=on -DMOD_REDIS=on -DMOD_SDORM_MONGO=on .
...
cd /root/ffead-cpp
cp -rf ffead-cpp-7.0-bin /root/ffead-cpp-7.0
rm -rf ffead-cpp-7.0-bin

cd /root/ffead-cpp-7.0
...
cd /root/ffead-cpp/lang-server-backends/rust/rocket-ffead-cpp
cargo build
cp target/debug/rocket-ffead-cpp /root/ffead-cpp/ffead-cpp-7.0-bin/
