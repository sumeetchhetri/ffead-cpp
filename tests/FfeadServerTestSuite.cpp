...
std::string execTest(strVecVec testCases, int var, std::string hdr) {
	std::string retHdrVal;
	Timer timer;
	...
	if(testCases[var].size()>=4)
	{
		...
		if(hdr.length()>0) {
			retHdrVal = res.getHeader(StringUtil::toLowerCopy(hdr));
		}
...
int main()
{
	...
	props = propFileReader.getProperties("testValues.prop");
	...
	{
		std::string pd = execTest(testCases, var, "Date");
		sleep(2);
		std::string cd = execTest(testCases, var, "Date");
		if(cd==pd) {
			std::cout << "Failed Date header cache validation pd -> " << pd << " cd -> " << cd << std::endl;
			return 1;
		} else {
			std::cout << "Passed Date header cache validation pd -> " << pd << " cd -> " << cd << std::endl;
		}
	}
